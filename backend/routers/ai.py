...
async def parse_po(file: UploadFile = File(...), current: dict = Depends(require_write)):
    ...
    # Track PDF/image auto-read in temp storage
    try:
        from routers.storage import record_temp_upload
        await record_temp_upload(current, "po_auto_read", file.filename or "po.pdf", content,
                                  mime=file.content_type or "", related_entity="Auto-Read PO")
    except Exception:
        pass
