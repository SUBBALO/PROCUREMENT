...
# ---------------- Laporan Belanja per Vendor (Bulanan) ----------------
import re as _re


def _valid_month(month: str) -> None:
    if not _re.match(r"^\d{4}-\d{2}$", month or ""):
        raise HTTPException(status_code=400, detail="Format bulan harus YYYY-MM (contoh: 2026-06)")


@router.get("/reports/vendor-monthly")
async def report_vendor_monthly(
    month: str = Query(..., description="Bulan periode YYYY-MM"),
    current: dict = Depends(get_current_user),
):
    """Ringkasan belanja per vendor (PT/supplier) dalam 1 bulan.
    Untuk tiap vendor: total belanja (IDR), jumlah transaksi, jumlah PO unik, jumlah item unik.
    Diurutkan dari total belanja terbesar."""
    _valid_month(month)
    match = merged({"invoice_date": {"$regex": f"^{month}"}}, NOT_DELETED_FILTER)
    pipeline = [
        {"$match": match},
        {"$group": {
            "_id": "$vendor_name",
            "total_idr": {"$sum": {"$ifNull": ["$total_price_idr", "$total_price"]}},
            "tx_count": {"$sum": 1},
            "qty_total": {"$sum": {"$ifNull": ["$qty", 0]}},
            "po_set": {"$addToSet": "$po_no"},
            "items": {"$addToSet": "$item_name"},
        }},
        {"$sort": {"total_idr": -1}},
    ]
    rows = await db.transactions.aggregate(pipeline).to_list(length=10000)
    vendors = []
    grand_total = 0.0
    grand_tx = 0
    for r in rows:
        po_count = len([p for p in (r.get("po_set") or []) if p and str(p).strip()])
        item_count = len([i for i in (r.get("items") or []) if i])
        total = float(r.get("total_idr") or 0)
        grand_total += total
        grand_tx += r.get("tx_count") or 0
        vendors.append({
            "vendor": r["_id"] or "(Tanpa Nama Vendor)",
            "total_idr": total,
...
@router.get("/reports/vendor-monthly/detail")
async def report_vendor_monthly_detail(
    month: str = Query(..., description="Bulan periode YYYY-MM"),
    vendor: str = Query(..., description="Nama vendor persis"),
    current: dict = Depends(get_current_user),
):
    """Rincian lengkap semua transaksi 1 vendor dalam 1 bulan (untuk drill-down)."""
    _valid_month(month)
    vendor_filter = vendor if vendor != "(Tanpa Nama Vendor)" else {"$in": ["", None]}
    match = merged({"invoice_date": {"$regex": f"^{month}"}, "vendor_name": vendor_filter}, NOT_DELETED_FILTER)
    txs = await db.transactions.find(match, {"_id": 0}).sort("invoice_date", 1).to_list(length=10000)
    subtotal = sum(float(t.get("total_price_idr") or t.get("total_price") or 0) for t in txs)
    return {
        "month": month,
        "vendor": vendor,
        "count": len(txs),
