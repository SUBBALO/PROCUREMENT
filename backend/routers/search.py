...
async def so_timeline(so_no: str, current: dict = Depends(get_current_user)):
    ...
    # Drawing Requests (Engineering) untuk SO ini
    drfs = await db.drawing_requests.find(
        merged({"so_no": so_no_norm}, NOT_DELETED_FILTER), {"_id": 0}
    ).sort("created_at", 1).to_list(length=200)

    # Laporan Produksi harian untuk SO ini
    prods = await db.production_reports.find(
        merged({"so_no": so_no_norm}, NOT_DELETED_FILTER), {"_id": 0}
    ).sort("report_date", 1).to_list(length=2000)

    # Document Control (controlled drawings / register) untuk SO ini
    dwgs = await db.drawings.find(
        merged({"so_no": so_no_norm}, NOT_DELETED_FILTER), {"_id": 0}
    ...
    _DRF_STATUS_ID = {
        "draft": "Draft", "submitted": "Terkirim ke Engineering", "accepted": "Diterima Engineering",
        "received": "Diterima Engineering", "in_progress": "Dikerjakan Engineering",
        "completed": "Selesai Engineering", "revision_requested": "Revisi diminta",
    }
    for dr in drfs:
        st = dr.get("status", "-")
        events.append({
            "when": dr.get("created_at") or dr.get("date") or "",
            "type": "drawing",
            "icon": "ruler",
            "title": f"Drawing Request {dr.get('form_no', '(draft)')}",
            "detail": f"{dr.get('project_name', '')} — {_DRF_STATUS_ID.get(st, st)}",
            "actor": dr.get("requested_by_name") or dr.get("created_by_name") or "-",
            "link": "/engineering",
            "link_id": dr.get("id"),
        })
    for p in prods:
        ok = p.get("qty_ok", 0)
        ng = p.get("qty_ng", 0)
        proc = p.get("process") or p.get("operator_name") or ""
        events.append({
            "when": p.get("report_date") or p.get("created_at") or "",
            "type": "production",
            "icon": "factory",
            "title": f"Produksi — {p.get('operator_name', '-')}",
            "detail": f"{proc} · OK {ok}" + (f" · NG {ng}" if ng else ""),
            "actor": p.get("created_by_username") or p.get("operator_name") or "-",
            "link": "/produksi/daily-report",
            "link_id": p.get("id"),
        })
    for dw in dwgs:
        appr = dw.get("approval_status")
        events.append({
            "when": dw.get("updated_at") or dw.get("created_at") or "",
            "type": "doccon",
            "icon": "stamp",
            "title": f"Drawing {dw.get('drawing_no', '-')} ({dw.get('revision', '-')})",
            "detail": f"Status: {dw.get('status', '-')}" + (f" · Approval: {appr}" if appr else ""),
            "actor": dw.get("prepared_by") or dw.get("assigned_to_name") or "-",
            "link": "/controlled-drawings",
            "link_id": dw.get("id"),
        })
