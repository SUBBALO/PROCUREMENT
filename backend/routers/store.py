...
async def store_receive_bulk(payload: BulkReceiveRequest, current: dict = Depends(require_store_access)):
    ...
    # Auto-create QC MII inspection for items NOT going to stock
    try:
        from routers.qc import create_inspection_from_receipts
        first = received_docs[0]
        await create_inspection_from_receipts(
            receipt_docs=received_docs,
            source_type="supplier",  # PO-based receiving = from supplier
            source_name=first.get("vendor_name", ""),
            do_no=payload.do_number or "",
            po_no=first.get("po_no", ""),
            receive_date=payload.receive_date,
            current=current,
        )
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"QC auto-create failed: {e}")
...
async def store_stock(current: dict = Depends(require_store_access),
    ...
    # Sertakan juga item yg sudah habis (qty_remaining=0) supaya user bisa cek history item kosong.
    # Item habis akan muncul di paling bawah (sort di frontend).
    match: dict = {}
    if customer_only:
...
async def stock_item_history(
    ...
    if is_customer_material is True:
        # Only customer-supplied material (explicit True)
        filt["is_customer_material"] = True
    elif is_customer_material is False:
        # "Regular" material: field missing OR false (legacy docs may not have it)
        filt["$or"] = [
            {"is_customer_material": {"$exists": False}},
            {"is_customer_material": False},
            {"is_customer_material": None},
        ]
...
async def store_incoming(payload: IncomingGoodsRequest, current: dict = Depends(require_store_write)):
    ...
    try:
        ...
        # Do not block receiving on QC-create failure
        import logging
        logging.getLogger(__name__).warning(f"QC auto-create failed: {e}")
