...
FORM_CODES = {
    ...
    "MII": {
        ...
        "fields": [
            "company_name", "receive_date", "vendor_name", "po_no",
            "do_number", "invoice_no", "print_date", "printed_by",
            "qc_inspector", "inspection_date", "inspection_result",
        ],
        "table_key": "items",
        "table_fields": [
            "__index__", "so_no", "item_name", "qty_received", "unit",
...
def _xlsx_to_pdf(xlsx_bytes: bytes) -> bytes:
    """Convert xlsx bytes to PDF bytes via LibreOffice headless. Cross-platform:
    works on Windows, macOS, and Linux as long as LibreOffice is installed.

    On Windows you can override the path via env `SOFFICE_BIN`, e.g.
