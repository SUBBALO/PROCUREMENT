...
export default function StoreStockPage() {
  ...
  const sortedFiltered = useMemo(() => sortItems(filtered, sortBy, STOCK_SORT_OPTS), [filtered, sortBy]);
          ...
          <table className="w-full text-sm">
            ...
            <tbody data-testid="stock-table">
              ...
              {sortedFiltered.map((s) => (
                <tr
                  key={s.item_name}
                  className={`border-b border-slate-100 ${s.is_below_min ? "bg-red-50 hover:bg-red-100" : "hover:bg-slate-50"}`}
                  data-testid={`stock-row-${s.item_name.replace(/\s+/g, "-")}`}
                >
                  <td className="p-3 text-slate-900 max-w-[360px] font-medium">
                    {s.is_below_min && <Warning size={12} weight="fill" className="inline text-red-600 mr-1.5" />}
                    <button
                  ...
                  <td className={`p-3 text-right tabular-nums font-semibold ${s.is_below_min ? "text-red-700" : "text-sky-700"}`}>
