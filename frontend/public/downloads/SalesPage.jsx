...
const ROLE_ENG_ANY = ["engineering", "eng_head", "eng_staff"];
const ROLE_ENG_HEAD = ["engineering", "eng_head"];  // includes legacy
...
function InquiryDetailDialog({ inquiryId, user, onClose, onChanged, onEditDraft }) {
  ...
  const isEngOnly = ["engineering", "eng_head", "eng_staff"].includes(role);  // pure engineering (no sales privileges)
  ...
  const isEng = ["engineering", "eng_head", "eng_staff"].includes(role) || isAdminLike;
  const isEngHead = ["engineering", "eng_head"].includes(role) || isAdminLike;
  const isEngStaff = role === "eng_staff";
  ...
  // Eng Head who is NOT the assignee should not see the "Selesai" button.
      ...
      <DialogContent className="rounded-none max-w-4xl max-h-[92vh] overflow-y-auto">
        {loading || !data ? (
          ...
          <>
            ...
            {data.head_revision_note && (data.status === "head_revision" || data.status === "pending_head_review" || data.status === "awaiting_review") && (
              <div className="mt-2 p-2.5 border-l-4 border-orange-500 bg-orange-50 text-sm text-slate-800" data-testid="head-revision-note-banner">
                <div className="text-[10px] uppercase tracking-[0.1em] font-bold text-orange-700 mb-1">
                  ...
                  Revisi Diminta oleh {data.head_reviewed_by_name || "Eng Head"}{data.head_reviewed_at ? ` · ${data.head_reviewed_at.slice(0,16).replace('T',' ')}` : ""}
                </div>
            ...
            {data.head_review_note && data.head_review_note !== "(auto: engineer adalah Eng Head sendiri)" && (data.status === "awaiting_review" || data.status === "accepted" || data.status === "revision_requested" || data.status === "closed") && (
              <div className="mt-2 p-2.5 border-l-4 border-emerald-500 bg-emerald-50 text-sm text-slate-800" data-testid="head-approve-note-banner">
                <div className="text-[10px] uppercase tracking-[0.1em] font-bold text-emerald-700 mb-1">
                  ✓ Disetujui Eng Head {data.head_reviewed_by_name ? `— ${data.head_reviewed_by_name}` : ""}
                </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-200 space-y-2">
              ...
              {/* Engineer (assignee ONLY): Selesai Costing → Kirim ke Eng Head */}
                  ...
                  <Button data-testid="btn-complete-inquiry" onClick={() => setAction("complete")} className="rounded-none bg-emerald-600 hover:bg-emerald-700 text-white text-xs uppercase tracking-[0.1em]">
                    ...
                    {isEngHead && data.assigned_to_id === user?.id
                      ...
                      : data.status === "head_revision"
                        ? "Selesai Revisi & Kirim ke Eng Head"
                        : "Selesai Costing & Kirim ke Eng Head"}
                  </Button>
                </div>
              )}
              {action === "complete" && (
                <div className="p-3 border-2 border-emerald-400 bg-emerald-50 space-y-2" data-testid="complete-form">
                  ...
                  <Label className="text-xs font-semibold text-emerald-900">
                    {isEngHead && data.assigned_to_id === user?.id ? "Catatan untuk Sales" : "Catatan untuk Eng Head"}
                  </Label>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button data-testid="confirm-complete" onClick={() => doAction()} disabled={processing} className="rounded-none bg-emerald-600 text-white">
                      {processing ? "Mengirim..." : (isEngHead && data.assigned_to_id === user?.id ? "Kirim ke Sales" : "Kirim ke Eng Head")}
                    </Button>
                  </div>
                </div>
              )}

              {/* Eng Head: Review costing dari staff (status: pending_head_review) */}
        ...
        >
          ...
          {engineers.map((eng) => (
            <option key={eng.id} value={eng.id}>
              {eng.name || eng.username} ({eng.role === "eng_head" ? "Head" : eng.role === "eng_staff" ? "Staff" : "Engineering"})
            </option>
          ))}
        </select>
      )}
