...
const DO_SORT_OPTS = [
  { value: "date_desc", label: "Tanggal: Baru → Lama", sort: (a, b) => cmpDateStr(b.date, a.date) },
  { value: "date_asc", label: "Tanggal: Lama → Baru", sort: (a, b) => cmpDateStr(a.date, b.date) },
  { value: "dest_asc", label: "Nama Tujuan: A → Z", sort: (a, b) => cmpStr(a.destination, b.destination) },
  { value: "dest_desc", label: "Nama Tujuan: Z → A", sort: (a, b) => cmpStr(b.destination, a.destination) },
  { value: "gp_asc", label: "No Gatepass: A → Z", sort: (a, b) => cmpStr(a.gate_pass_no, b.gate_pass_no) },
  { value: "gp_desc", label: "No Gatepass: Z → A", sort: (a, b) => cmpStr(b.gate_pass_no, a.gate_pass_no) },
];
...
export default function DeliveryPage() {
  ...
  const sortedItems = useMemo(() => sortItems(filteredItems, sortBy, DO_SORT_OPTS), [filteredItems, sortBy]);
  const pag = usePagination(sortedItems, 20);

  // Flatten to one row per item — use PAGED items so table honors pagination
  const flatRows = [];
  pag.pagedData.forEach((d) => {
...
<div className="flex items-end justify-between gap-4 flex-wrap">
      ...
      <Card className="rounded-none border-slate-200 shadow-none p-4 bg-white">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div>
            ...
            <SortDropdown testid="delivery-sort" value={sortBy} onChange={setSortBy} options={DO_SORT_OPTS} className="w-full" />
          </div>
        </div>
      </Card>
      ...
      <Card className="rounded-none border-slate-200 shadow-none bg-white overflow-hidden">
        ...
        <PaginationBar {...pag} label="pengiriman" testIdPrefix="del-pag" />
      </Card>
...
function AddDeliveryDialog({ open, onClose, onSaved, sos }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="rounded-none max-w-4xl max-h-[90vh] overflow-y-auto" data-testid="add-delivery-dialog">
        <DialogHeader>
