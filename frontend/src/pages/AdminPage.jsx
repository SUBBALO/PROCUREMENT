...
export default function AdminPage() {
  ...
  // Only super_admin can access Admin Panel. Regular admins (e.g. Erwin) are blocked
  // even though they have approval permissions — they still see the approval banner elsewhere.
...
function CreateUserDialog({ open, onClose, onSaved }) {
  const [form, setForm] = useState({ username: "", password: "", name: "", role: "staff", view_store_report: false, approve_store_requests: false, access: {} });
  const [saving, setSaving] = useState(false);
  useEffect(() => { if (open) setForm({ username: "", password: "", name: "", role: "staff", view_store_report: false, approve_store_requests: false, access: {} }); }, [open]);
  ...
  const save = async () => {
    ...
    try {
      ...
      await api.post("/users", { username: form.username, password: form.password, name: form.name, role: form.role, perms, access: form.role === "hrd" ? (form.access || {}) : {}, must_change_password: form.role === "hrd" });
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="rounded-none max-h-[90vh] overflow-y-auto" data-testid="create-user-dialog">
        <DialogHeader>
        ...
        <div className="grid gap-3">
          ...
          <div>
            ...
            <Select value={form.role} onValueChange={(v) => setForm({ ...form, role: v })}>
              ...
              <SelectContent>
                ...
                <SelectItem value="hrd">HRD (Portal HRD — atur akses di bawah)</SelectItem>
              </SelectContent>
            </Select>
            <div className="text-[11px] text-slate-500 mt-1 leading-relaxed">
              <b>Admin</b>: full access · <b>Supervisor</b>: bisa approve tanpa panel admin · <b>Sales</b>: Sales dept only<br />
              <b>Engineering Leader</b>: Riski/Zul — assign job ke engineer · <b>Eng Staff</b>: hanya kerjakan job yg di-assign, tapi tetap bisa upload BOM<br />
              <b>Purchasing</b>: input transaksi · <b>Store</b>: gudang · <b>Finance</b>: view + export
            </div>
          </div>
          ...
          {form.role === "hrd" && <HrdAccessMatrix value={form.access} onChange={(a) => setForm({ ...form, access: a })} />}
        </div>
        <DialogFooter>
...
const HRD_ACTIONS_DEF = [["view", "Lihat"], ["create", "Tambah"], ["edit", "Ubah"], ["delete", "Hapus"], ["report", "Cetak"]];

function HrdAccessMatrix({ value, onChange }) {
  const acc = value || {};
  const toggle = (menu, action) => {
    const cur = { ...(acc[menu] || {}) };
    cur[action] = !cur[action];
    if (action !== "view" && cur[action]) cur.view = true; // aksi butuh minimal lihat
    onChange({ ...acc, [menu]: cur });
  };
  const setRow = (menu, val) => {
    onChange({ ...acc, [menu]: HRD_ACTIONS_DEF.reduce((o, [a]) => ({ ...o, [a]: val }), {}) });
  };
  return (
                ...
                <tr key={m.key}>
                  ...
                  {HRD_ACTIONS_DEF.map(([a]) => (
                    <td key={a} className="px-2 py-1.5 text-center">
                      <input type="checkbox" className="w-4 h-4 accent-teal-600" checked={!!row[a]}
                        onChange={() => toggle(m.key, a)} data-testid={`hrd-perm-${m.key}-${a}`} />
                    </td>
                  ))}
    ...
    <Dialog open={!!user} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="rounded-none max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-3">
          ...
          <div>
            ...
            <Select value={form.role} onValueChange={(v) => setForm({ ...form, role: v })} disabled={isSelf}>
              ...
              <SelectContent>
                ...
                <SelectItem value="hrd">HRD (Portal HRD)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
          ...
          <SignatureUploadBlock userId={user.id} />
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose} className="rounded-none">Batal</Button>
...
function SignatureUploadBlock({ userId }) {
  ...
  useEffect(() => {
    // Test existence by trying to load
    let cancelled = false;
    api.get(`/users/${userId}/signature`, { responseType: "blob" })
      .then((res) => {
        if (cancelled) return;
      ...
      .catch(() => { if (!cancelled) { setHasSignature(false); setPreview(null); } });
  ...
  const onFile = async (e) => {
    ...
    setUploading(true);
    try {
      await api.post(`/users/${userId}/signature`, fd, { headers: { "Content-Type": "multipart/form-data" } });
      toast.success("TTD berhasil diupload");
      setRefreshKey(k => k + 1);
    } catch (err) { toast.error(err.response?.data?.detail || "Gagal upload"); } finally { setUploading(false); }
  };

  const doDelete = async () => {
    if (!window.confirm("Hapus TTD tersimpan?")) return;
    try {
      await api.delete(`/users/${userId}/signature`);
      toast.success("TTD dihapus");
      setPreview(null); setHasSignature(false);
    } catch (e) { toast.error(e.response?.data?.detail || "Gagal hapus"); }
  };

  return (
    <div className="border-t border-slate-200 pt-3">
      <div className="text-xs font-bold text-slate-700 uppercase tracking-[0.1em] mb-2">Tanda Tangan (TTD)</div>
      <div className="flex items-start gap-4">
        <div className="w-40 h-24 border-2 border-dashed border-slate-300 bg-slate-50 flex items-center justify-center">
          {preview ? (
            <img src={preview} alt="TTD" className="max-w-full max-h-full object-contain" />
          ) : (
            <span className="text-[10px] uppercase tracking-[0.1em] text-slate-400">Belum ada TTD</span>
          )}
        </div>
        <div className="flex-1 space-y-2">
          <label className="inline-block cursor-pointer">
            <span className="inline-flex items-center gap-1 h-8 px-3 border border-sky-400 text-sky-700 hover:bg-sky-50 text-xs font-bold uppercase tracking-[0.1em]">
              {uploading ? "Uploading..." : hasSignature ? "Ganti TTD" : "Upload TTD"}
            </span>
            <input type="file" accept="image/png,image/jpeg,image/webp" onChange={onFile} className="hidden" disabled={uploading} data-testid={`upload-sig-${userId}`} />
          </label>
          {hasSignature && (
            <Button size="sm" variant="ghost" onClick={doDelete} className="rounded-none text-xs text-red-600 hover:bg-red-50 ml-2">Hapus</Button>
          )}
          <div className="text-[10px] text-slate-500 leading-relaxed">
            PNG/JPG/WebP · Max 2MB · Disarankan <b>PNG background transparan</b>, ukuran ~400×200px.<br />
            TTD ini akan muncul di PDF Quotation yang dibuat oleh user ini.
          </div>
        </div>
      </div>
    </div>
...
function LogsTab() {
  ...
  const [filters, setFilters] = useState({ user_id: "", action: "", entity: "", start_date: "", end_date: "" });
    ...
    <div className="space-y-4">
      <Card className="rounded-none border-slate-200 shadow-none p-4 bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div>
            <Label className="text-xs font-semibold text-slate-600 mb-1 block">Modul</Label>
            <Select value={filters.entity || "all"} onValueChange={(v) => setFilters({ ...filters, entity: v === "all" ? "" : v })}>
              <SelectTrigger data-testid="log-entity-filter" className="rounded-none h-9 border-slate-300 text-sm"><SelectValue /></SelectTrigger>
              <SelectContent>
