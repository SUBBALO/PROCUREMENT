...
// File bersama single (1 per box / BOM)
const SINGLE_SLOTS = [
  { key: "nesting",       label: "Nesting",        accept: ".pdf,.doc,.docx,.xlsx,.xls", hint: "PDF/Word/Excel" },
  { key: "nesting_price", label: "Nesting Price",  accept: ".pdf,.doc,.docx,.xlsx,.xls", hint: "PDF/Excel" },
];

const SHARED_FIELDS = [
  { key: "so_no", label: "No. SO (6 digit)", placeholder: "005500", numeric: true },
  { key: "project_name", label: "Nama Project", placeholder: "..." },
  { key: "customer", label: "Customer", placeholder: "SPM, PT" },
...
const newBox = () => ({
  uid: ++_uid,
  fields: emptyFields(),
  shared: { bom: [], nesting: null, nesting_price: null },
  dwgs: [emptyDwg()],
  items: [],
  itemsCount: 0,
  status: "idle",
...
const padSo = (so) => (so && /^\d{1,6}$/.test(so) ? so.padStart(6, "0") : so);
...
export default function LegacyImportPage() {
  ...
  const [dup, setDup] = useState({ open: false, box: null, info: null });
  ...
  const setSingle = (uid, slotKey, file) =>
    setBoxes((prev) => prev.map((b) => (b.uid === uid ? { ...b, shared: { ...b.shared, [slotKey]: file } } : b)));

  // BOM boleh lebih dari 1 file (lampiran). File pertama dipakai untuk auto-detect data.
  const addBomFiles = (uid, fileList) => {
    const files = Array.from(fileList || []);
    if (!files.length) return;
    const box = boxes.find((b) => b.uid === uid);
    const wasEmpty = !box || (box.shared.bom || []).length === 0;
    setBoxes((prev) => prev.map((b) => (b.uid === uid ? { ...b, shared: { ...b.shared, bom: [...(b.shared.bom || []), ...files] } } : b)));
    if (wasEmpty) analyzeBom(uid, files[0]);
  };
  const removeBomFile = (uid, index) =>
    setBoxes((prev) => prev.map((b) => (b.uid === uid ? { ...b, shared: { ...b.shared, bom: (b.shared.bom || []).filter((_, i) => i !== index) } } : b)));
  ...
  const validateBox = (box) => {
    ...
    if (!dwg1?.eng) { toast.error("File Eng DWG (DWG #1) wajib diupload"); return false; }
    if (!dwg1.drawing_no?.trim()) { toast.error("No. Eng DWG (DWG #1) wajib diisi"); return false; }
    for (let i = 1; i < box.dwgs.length; i++) {
      if (box.dwgs[i].eng && !box.dwgs[i].drawing_no?.trim()) { toast.error(`No. Eng DWG (DWG #${i + 1}) wajib diisi`); return false; }
    }
    return true;
  };

  // Step 1: cek duplikat. Jika ada → tampilkan dialog Update/Skip. Jika tidak → langsung commit.
  const commitBox = async (box) => {
    if (!validateBox(box)) return;
    patchBox(box.uid, { status: "checking", message: "Memeriksa data lama..." });
    try {
      const { data: chk } = await api.post("/legacy-import/check", {
        so_no: padSo(box.fields.so_no),
        bom_no: box.fields.bom_no,
        revision: box.fields.revision,
        drawing_no: box.dwgs[0].drawing_no,
      });
      if (chk.exists) {
        patchBox(box.uid, { status: "idle", message: `Sudah ada: ${chk.reason}` });
        setDup({ open: true, box, info: chk });
        return;
      }
    } catch (e) {
      // jika check gagal, tetap lanjut commit (backend akan tolak duplikat via 409)
    }
    await doCommit(box, "create");
  };

  const doCommit = async (box, mode) => {
    patchBox(box.uid, { status: "saving", message: mode === "update" ? "Memperbarui data lama..." : "Menyimpan ke Master List..." });
    try {
      const so = padSo(box.fields.so_no);
      const dwg1 = box.dwgs[0];

      // 1) commit DWG#1 + BOM(multi) + Nesting/Nesting Price
      const fd = new FormData();
      const meta = {
        ...box.fields, so_no: so,
        drawing_no: dwg1.drawing_no, customer_drawing_no: dwg1.customer_drawing_no,
        items: box.items, mode,
      };
      ...
      (box.shared.bom || []).forEach((f) => fd.append("bom_files", f));
      const { data } = await api.post("/legacy-import/commit", fd, { headers: { "Content-Type": "multipart/form-data" } });
      const bomId = data.bom_id;

      // 2) DWG tambahan → add-drawing (drawing terpisah, BOM/SO sama). Abaikan yang sudah ada.
      let extra = 0;
      for (let i = 1; i < box.dwgs.length; i++) {
        const d = box.dwgs[i];
        if (!d.eng) continue;
        try {
          ...
          // 409 = drawing tambahan sudah ada → lewati saat update
          if (err?.response?.status !== 409) throw err;
        }
      }
      const total = 1 + extra;
      const verb = data.updated ? "diperbarui" : "masuk Master List";
      patchBox(box.uid, { status: "saved", message: `${total} drawing ${verb} (SO ${data.so_no})`, saved: { ...data, total } });
      toast.success(`${total} drawing (SO ${data.so_no}) ${verb}`);
    } catch (e) {
      const raw = e?.response?.data?.detail;
      const msg = (raw && typeof raw === "object" ? raw.message : raw) || "Gagal menyimpan";
      patchBox(box.uid, { status: "error", message: msg });
      toast.error(msg);
    }
  };

  // Dialog duplikat: Update / Skip
  const handleUpdate = () => {
    const box = dup.box;
    setDup({ open: false, box: null, info: null });
    if (box) doCommit(box, "update");
  };
  const handleSkip = () => {
    const box = dup.box;
    if (box) patchBox(box.uid, { status: "saved", message: "Dilewati — data sudah ada di sistem", saved: { skipped: true, so_no: padSo(box.fields.so_no), total: 0, bom_no: dup.info?.bom_no } });
    setDup({ open: false, box: null, info: null });
    toast.info("Import dilewati (skip).");
  };
  const handleCancelDup = () => setDup({ open: false, box: null, info: null });
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8" data-testid="legacy-import-page">
      ...
      <div className="mt-4 mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <Stack size={26} weight="duotone" className="text-slate-700" /> Import Data Lama &rarr; Master List
          </h1>
          <p className="text-sm text-slate-500 mt-1 max-w-2xl">
            Upload <b>BOM (Excel)</b> dulu &mdash; boleh <b>lebih dari 1 file</b> (lampiran) &rarr; sistem auto-baca data &rarr; lalu upload DWG.
            Bisa <b>lebih dari 1 DWG</b> (tiap DWG jadi drawing terpisah, SO/BOM sama). Verifikasi lalu simpan. Hasil langsung <b>Controlled/Final</b>.
          </p>
        </div>
      ...
      <div className="space-y-5">
        {boxes.map((box, idx) => {
          const busy = box.status === "saving" || box.status === "analyzing" || box.status === "checking";
          return (
            ...
            <div className="p-4 md:p-5">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {(box.shared.bom || []).length > 0 && <Badge variant="outline" className="rounded-none border-emerald-400 text-emerald-700">{box.shared.bom.length} file BOM</Badge>}
                  ...
                  {box.status === "saved" && !box.saved?.skipped && <Badge className="rounded-none bg-emerald-600 text-white flex items-center gap-1"><CheckCircle size={13} weight="fill" /> {box.saved?.updated ? "Diperbarui" : "Masuk Master List"}</Badge>}
                  {box.status === "saved" && box.saved?.skipped && <Badge className="rounded-none bg-slate-500 text-white">Dilewati</Badge>}
                </div>
              ...
              <div className="bg-slate-50 border border-slate-200 p-3 mb-4">
                <div className="flex items-center gap-2 mb-3 text-xs uppercase tracking-wider font-semibold text-slate-500">
                  <MagicWand size={14} weight="duotone" /> Data BOM {box.status === "analyzing" && <span className="text-amber-600 normal-case">&middot; membaca...</span>}
                </div>
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="text-[11px] uppercase tracking-wider font-semibold text-slate-500">Drawing (Eng DWG + Customer) &mdash; bisa lebih dari 1</div>
                {box.status !== "saved" && (
              ...
              <div className="flex items-center justify-between">
                ...
                {box.status !== "saved" ? (
                  <Button onClick={() => commitBox(box)} disabled={busy} className="rounded-none bg-emerald-600 hover:bg-emerald-700" data-testid={`commit-box-${idx}`}>
                    <FloppyDisk size={16} weight="bold" className="mr-1" /> {box.status === "saving" ? "Menyimpan..." : box.status === "checking" ? "Memeriksa..." : "Verifikasi & Simpan ke Master List"}
                  </Button>
                ) : (
                  <span className="text-xs text-emerald-700 font-semibold flex items-center gap-1">
                    <CheckCircle size={15} weight="fill" /> SO {box.saved?.so_no} &middot; {box.saved?.skipped ? "dilewati" : `${box.saved?.total} drawing`} {box.saved?.bom_no ? `· ${box.saved.bom_no}` : ""}
                  </span>
                )}
              </div>
            </div>
      ...
      {/* Dialog duplikat: Update / Skip */}
      <AlertDialog open={dup.open} onOpenChange={(o) => { if (!o) handleCancelDup(); }}>
        <AlertDialogContent className="rounded-none" data-testid="dup-dialog">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2 text-amber-700">
              <WarningCircle size={22} weight="fill" /> Data sudah ada di sistem
            </AlertDialogTitle>
            <AlertDialogDescription className="text-slate-600">
              <span className="font-semibold text-slate-800">{dup.info?.reason}</span> sudah tercatat.
              {dup.info?.drawing_count ? ` Saat ini ada ${dup.info.drawing_count} drawing tertaut.` : ""}
              <br />Pilih <b>Update</b> untuk memperbarui data lama (item BOM, metadata, file DWG diganti, lampiran ditambahkan),
              atau <b>Skip</b> untuk melewati import ini.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2">
            <Button variant="outline" className="rounded-none" onClick={handleCancelDup} data-testid="dup-cancel">Batal</Button>
            <Button variant="outline" className="rounded-none border-slate-400 text-slate-700" onClick={handleSkip} data-testid="dup-skip">Skip</Button>
            <Button className="rounded-none bg-amber-600 hover:bg-amber-700" onClick={handleUpdate} data-testid="dup-update">
              <ArrowClockwise size={15} weight="bold" className="mr-1" /> Update Data Lama
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
...
function BomMultiSlot({ files, onAdd, onRemove, disabled, testid }) {
  const inputId = `bom-${Math.random().toString(36).slice(2, 8)}`;
  return (
    <div>
      <Label className="text-[11px] font-semibold text-slate-500 mb-1 flex items-center gap-1">
        BOM (Excel) <span className="text-[9px] text-slate-400 normal-case">&mdash; boleh lebih dari 1</span>
      </Label>
      {files.length === 0 ? (
        <label htmlFor={inputId}
          ...
          <span className="text-[10px] text-slate-400 leading-tight">Excel &mdash; auto-detect</span>
          <input id={inputId} type="file" accept=".xlsx,.xls,.xlsm" multiple className="hidden" disabled={disabled}
            onChange={(e) => { if (e.target.files?.length) onAdd(e.target.files); e.target.value = ""; }} />
        </label>
      ) : (
        <div className="border-2 border-slate-800 bg-white p-2 min-h-20 flex flex-col gap-1" data-testid={`${testid}-selected`}>
          <div className="flex flex-col gap-1 max-h-24 overflow-auto">
            {files.map((f, i) => (
              <div key={i} className="flex items-center gap-1.5 text-[10px] text-slate-700" data-testid={`${testid}-file-${i}`}>
                {fileIcon(f.name)}
                <span className="break-all line-clamp-1 flex-1">{f.name}</span>
                <span className="text-[9px] text-slate-400 shrink-0">{(f.size / 1024).toFixed(0)}KB</span>
                {!disabled && <button onClick={() => onRemove(i)} className="text-slate-400 hover:text-rose-600 shrink-0" title="Hapus"><X size={12} weight="bold" /></button>}
              </div>
            ))}
          </div>
          {!disabled && (
            <label htmlFor={inputId} className="mt-auto inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700 cursor-pointer hover:text-emerald-800">
              <Plus size={12} weight="bold" /> Tambah file BOM
              <input id={inputId} type="file" accept=".xlsx,.xls,.xlsm" multiple className="hidden"
                onChange={(e) => { if (e.target.files?.length) onAdd(e.target.files); e.target.value = ""; }} />
            </label>
          )}
        </div>
