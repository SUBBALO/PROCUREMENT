...
export default function MySignatureHistoryPage() {
  ...
  const filtered = items.filter((h) => {
    ...
    if (q) {
      ...
      return (
        (h.drawing_no || "").toLowerCase().includes(s) ||
        (h.project_name || "").toLowerCase().includes(s) ||
        (h.customer_name || "").toLowerCase().includes(s) ||
        (h.so_no || "").toLowerCase().includes(s)
      );
    }
    return true;
  });
  ...
  // Statistik ringkas
  const stats = items.reduce((acc, h) => {
    acc.total = (acc.total || 0) + 1;
    acc[h.stage] = (acc[h.stage] || 0) + 1;
    return acc;
  }, {});
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            ...
            Klik <b>Preview PDF</b> untuk lihat drawing lengkap dengan TTD Anda + tanggal + posisi.
          </p>
        </div>
        <button
          onClick={() => window.print()}
          className="px-3 py-2 text-xs font-bold uppercase tracking-widest bg-slate-900 text-white hover:bg-slate-800 flex items-center gap-1.5"
          data-testid="sig-history-print"
        >
      ...
      <Card className="rounded-none border-slate-200 p-3 flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-[240px]">
          <input
            ...
            placeholder="Cari drawing no / project / SO / customer..."
            className="w-full border border-slate-300 px-3 py-2 text-sm rounded-none focus:outline-none focus:border-slate-500"
            data-testid="sig-history-search"
          />
        </div>
        <select
          value={stageFilter}
          onChange={(e) => setStageFilter(e.target.value)}
          className="border border-slate-300 px-3 py-2 text-sm rounded-none focus:outline-none focus:border-slate-500"
          data-testid="sig-history-stage-filter"
        >
        ...
        <button
          onClick={load}
          className="px-3 py-2 border border-slate-300 hover:bg-slate-50 text-xs font-bold uppercase tracking-widest flex items-center gap-1.5"
          data-testid="sig-history-refresh"
        >
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={7} className="p-6 text-center text-slate-400">
                <ArrowClockwise size={20} className="mx-auto animate-spin" /> Memuat...
              </td></tr>
            )}
              ...
              return (
                <tr key={`${h.drawing_id}-${h.stage}-${i}`}
                  ...
                  <td className="p-3 text-xs">
                    <div className="text-slate-800">{h.project_name || "-"}</div>
                    <div className="text-slate-500">{h.customer_name || ""}</div>
                  </td>
                  <td className="p-3 font-mono text-xs text-slate-700">{h.so_no || "-"}</td>
                  <td className="p-3 text-center">
                    <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider border ${stage.cls}`}>
                      {stage.text}
                    </span>
                  </td>
                  <td className="p-3 text-center text-xs text-slate-600">
                    {STATUS_LABEL[h.drawing_status_now] || h.drawing_status_now || "-"}
                  </td>
                  <td className="p-3 text-center">
                    <div className="flex gap-1 justify-center">
                      {h.has_pdf && (
                        <a
                          href={`${apiUrl}/api/drawings/${h.drawing_id}/pdf-stamped`}
                          target="_blank"
                          rel="noreferrer"
                          ...
                          data-testid={`sig-preview-${i}`}
                          title="Buka PDF drawing lengkap dengan TTD Anda"
                        >
                          <Eye size={11} weight="bold" /> Preview PDF
                        </a>
                      )}
                      <a
                        href={`/engineering/work-order/${h.drawing_id}`}
                        className="inline-flex items-center px-2 py-1 border border-slate-300 hover:bg-slate-100 text-slate-800 text-[10px] font-bold uppercase gap-0.5"
                      >
                        Detail
                      </a>
                    </div>
