...
export default function BOMPage() {
  ...
  const doUpload = async () => {
    ...
    try {
      ...
      setUploadOpen(false); setUploadFile(null); setUploadReason("");
      await loadList(searchSo);
    } catch (e) {
      ...
      <Card className="rounded-none border-slate-200 p-4">
        <form onSubmit={doSearch} className="flex items-end gap-3">
          <div className="flex-1 max-w-md">
            <Label className="text-xs font-semibold text-slate-600 mb-1 block">Cari Nomor SO</Label>
            <Input
              data-testid="bom-search-input"
              className={inputCls}
              value={searchSo}
              onChange={(e) => setSearchSo(e.target.value)}
              placeholder="mis. 005221"
            />
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-white border-b border-slate-200">
                <tr className="text-xs uppercase tracking-[0.1em] font-bold text-slate-500">
                  ...
                  <th className="text-left p-3">Diupload oleh</th>
      ...
      <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
        <DialogContent className="rounded-none max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Format: .xls atau .xlsx sesuai template MKS Engineering. Sistem otomatis membaca SO No & item.
              Jika SO sudah pernah diupload, akan disimpan sebagai revisi baru.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3">
            ...
            <div>
              <Label className="text-xs font-semibold text-slate-600 mb-1 block">Alasan Revisi <span className="text-slate-400 font-normal normal-case">(wajib jika SO sudah ada)</span></Label>
              <Input
            ...
            <div className="bg-amber-50 border border-amber-200 p-2 text-[11px] text-amber-800 flex gap-2">
              <Warning size={14} weight="fill" className="mt-0.5 shrink-0" />
              <span>Upload pertama tidak butuh alasan (auto-set Rev.0). Upload berikutnya untuk SO yang sama akan otomatis di-Rev.1, Rev.2, dst.</span>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setUploadOpen(false)} disabled={uploading} className="rounded-none">Batal</Button>
            <Button data-testid="bom-upload-submit" onClick={doUpload} disabled={uploading || !uploadFile} className="rounded-none bg-slate-900 hover:bg-slate-800 text-white">
              ...
              <tbody>
                {history.map((h) => (
                  <tr key={h.id} className="border-b border-slate-100">
                    ...
                    <td className="p-2 text-slate-700">{h.uploaded_by_name}</td>
          ...
          <table className="w-full text-sm" data-testid="bom-items-table">
            ...
            <tbody>
              {items.map((it) => {
                ...
                return (
                  <tr key={it.item_no} className="border-b border-slate-100 hover:bg-slate-50">
                    <td className="p-2 text-slate-400 tabular-nums">{it.item_no}</td>
                    <td className="p-2 font-semibold text-slate-900 whitespace-nowrap">{it.item_name}</td>
                    <td className="p-2 text-slate-700 max-w-[300px]">{it.item_specification}</td>
                    <td className="p-2 text-right tabular-nums">{it.qty}</td>
                    <td className="p-2 text-slate-600 text-xs">{it.uom}</td>
                    <td className="p-2 text-slate-600 text-xs">{it.material}</td>
                    <td className="p-2 text-slate-600 text-xs font-mono">{it.remark}</td>
                    ...
                    <td className="p-1 bg-amber-50/40">
                      {canAnnotate ? (
                        ...
                        <div className="text-xs text-slate-700 px-2 italic">{ann.admin_remark || "-"}</div>
                      )}
                    </td>
                  </tr>
                );
